...
def run_prediction():
    ...
    if selected_analysis == menu[0]:
        ...
        st.markdown("""
        ### 📌 성남시 반려동물 인프라 분석 앱에서의 K-Means 클러스터링
        이 앱에서는 **다음과 같은 데이터를 기반으로 K-Means 클러스터링을 수행**합니다.

        📍 **데이터 스케일링**  
        - **MinMaxScaler**를 활용하여 변수 간 크기 차이를 조정하여 최적의 클러스터링을 수행하도록 하였습니다.
        - 위도·경도는 공간적 특성을 고려하여 스케일링하지 않았습니다.

        📍 **클러스터링 기준**  
        - 🧭 **위도·경도** → 공간적 분포 고려  
        - 🐶 **동물소유자 수, 반려동물 수** → 수요 분석  
        - 🏥 **병원 수, 약국 수** → 공급 측면 분석  
        - 📊 **병원당·약국당 반려동물 수** → 밀집도 분석  

        이러한 분석을 통해 **반려동물 대비 병원·약국이 부족한 지역을 효과적으로 식별하여 신규 개업에 대한 인사이트를 제공합니다.**  
        """)
        ...
        for _, row in df.iterrows():
            ...
            folium.Marker(
                ...
                popup=folium.Popup(f'<div style="white-space: nowrap;"> {recommendation}<br> 클러스터: {row["클러스터"]}<br>동 이름: {row["지역명"]}<br> 병원 수: {int(row["병원수"])}<br>약국 수: {int(row["약국수"])}<br>등록 반려동물수: {int(row["반려동물수"])}</div>', max_width=300),
